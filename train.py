...
class Trainer:
    ...
    def _run_eval(self, epoch, step=0):
        ...
        filename = '{}/Halfway point_{}_{}.mdl'.format( self.args.model_dir, epoch, step )
        if step == 0:
            filename = '{}/Halfway point_epoch{}.mdl'.format( self.args.model_dir, epoch )
        save_checkpoint( {
            'epoch': epoch,
            'args': self.args.__dict__,
            'state_dict': self.model.state_dict(),
        }, is_best=is_best, filename=filename )
        delete_old_ckt( path_pattern='{}/Halfway point_*.mdl'.format( self.args.model_dir ),
    ...
    def eval_epoch(self, epoch) -> Dict:
        ...
        for i, batch_dict in enumerate( self.valid_loader ):
            ...
            hit1, acc3 = accuracy( logits, labels, topk=(1, 3) )
            top1.update( hit1.item(), batch_size )
    ...
    def train_epoch(self, epoch):
        ...
        for i, batch_dict in enumerate( self.train_loader ):
            ...
            outputs = get_model_obj( self.model ).compute_logits( output_dict=outputs, batch_dict=batch_dict )
            outputs = ModelOutput( **outputs )
            logits, labels, distances = outputs.logits, outputs.labels, outputs.distances
            ...
            losses.update( mean_loss.item(), batch_size )
