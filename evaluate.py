...
from token import load_data, Example
from link_prediction import BertPredictor
from dict import get_entity_dict, get_all_triplet_dict, get_link_graph
...
def rerank_by_graph(batch_score: torch.tensor,
    ...
    for idx in range(batch_score.size(0)):
        cur_ex = examples[idx]
        n_hop_indices = get_link_graph().get_n_hop_entity_indices(cur_ex.subject_id,
...
@torch.no_grad()
def compute_metrics(sr_tensor: torch.tensor,  
                    entities_tensor: torch.tensor,
                    target: List[int],
                    examples: List[Example],
                    k=3, batch_size=256) -> Tuple:
    assert sr_tensor.size(1) == entities_tensor.size(1)
    total = sr_tensor.size(0)
    entity_cnt = len(entity_dict)
    assert entity_cnt == entities_tensor.size(0)
    target = torch.LongTensor(target).unsqueeze(-1).to(sr_tensor.device)
    ...
    for start in tqdm.tqdm(range(0, total, batch_size)):
        ...
        batch_score = torch.mm(sr_tensor[start:end, :], entities_tensor.t())
        ...
        for idx in range(batch_score.size(0)):
            ...
            gold_neighbor_ids = all_triplet_dict.get_neighbors(cur_ex.subject_id, cur_ex.relation)
            if len(gold_neighbor_ids) > 10000:
                logger.debug('{} - {} has {} neighbors'.format(cur_ex.subject_id, cur_ex.relation, len(gold_neighbor_ids)))
            for e_id in gold_neighbor_ids:
                if e_id == cur_ex.object_id:
                    continue
...
def eval_single_direction(predictor: BertPredictor,   
    ...
    sr_tensor, _ = predictor.predict_by_examples(examples)
    sr_tensor = sr_tensor.to(entity_tensor.device)
    target = [entity_dict.entity_to_idx( ex.object_id ) for ex in examples]
    logger.info('predict tensor done, compute metrics...')

    topk_scores, topk_indices, metrics, ranks = compute_metrics(sr_tensor=sr_tensor, entities_tensor=entity_tensor,
    ...
    for idx, ex in enumerate(examples):
        ...
        pred_info = PredInfo( subject=ex.subject, relation=ex.relation,
                              object=ex.object, pred_object=entity_dict.get_entity_by_idx( pred_idx ).entity,
