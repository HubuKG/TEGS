...
from token import collate, Example, Dataset
...
class BertPredictor:
    ...
    def predict_by_examples(self, examples: List[Example]):
        ...
        sr_tensor_list, object_tensor_list = [], []
        for idx, batch_dict in enumerate(data_loader):
            if self.use_cuda:
                batch_dict = move_to_cuda(batch_dict)
            outputs = self.model(**batch_dict)
            sr_tensor_list.append(outputs['sr_vector'])
            object_tensor_list.append(outputs['object_vector'])

        return torch.cat(sr_tensor_list, dim=0), torch.cat(object_tensor_list, dim=0)

    @torch.no_grad()
    def predict_by_entities(self, entity_exs) -> torch.tensor:
        examples = []
        for entity_ex in entity_exs:
            examples.append( Example( subject_id='', relation='',
                                      object_id=entity_ex.entity_id ) )
        data_loader = torch.utils.data.DataLoader(
            Dataset(path='', examples=examples, task=args.task),
            num_workers=2,
